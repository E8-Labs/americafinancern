...
const HousingDurationScreen = ({ navigation, route }) => {
    ...
    const nextBtnAction = () => {
    const IntMinYear = Number(slected.minYears)
    const IntMaxYear = Number(slected.maxYears)
        ...
        housingData.min_living_year = IntMinYear,
            housingData.max_living_year = IntMaxYear,
...

                    {
                        durations.map((item) =>
                            <TouchableOpacity key={item.id} style={{ marginBottom: 18 / 852 * height }} onPress={() => {
                                ...
                                <View style={{
                                    height: 74 / 852 * height, width: 357 / 393 * width, borderRadius: 23/853 * height, borderWidth: 1, borderColor: '#cccccc', padding: 15,
                                    ...
                                    <View style={{
                                        height: 35 / 852 * height, width: 35 / 852 * height, borderRadius: 17/853 * height, alignItems: 'center',
